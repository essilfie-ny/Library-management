...
""")
    ...
    def search_by_title(self):
        search_t = input("\nEnter book title: ")
        #search_a = input("Enter the author of the book: ")
        found = False
        books = self.load_books()
        num = 0
        for book in books:
...
""")
        ...
        if found:
            ...
            while(True):
                ...
                try:
                    month = int(input("How month would you like to borrow book for(1-24): "))
...
Please note additional charges will be add if book is not return before due date in a readable condition""")
                        books.append(bookb)
                        self.save_book(books)
                    found = True
...
""")
    ...
    def return_borrowed_book(self):
        bookb = dict()
        while True:
            serial = input("Enter serial number on book: ")
            found = False
            books = self.load_books()
            for book in books:
                if book['serial'].lower() == serial:
                    bookb['title'] = book['title']
                    bookb['author'] = book['author']
                    bookb['serial'] = book['serial']
                    bookb['price'] = book['price']
            ...
            if not found:
                ...
                print("Please try again or senter 0 to return to main menu" )
        input("\nPress enter to continue\n") 
